    ...
    async function loadCatalog() {
      try {
        ...
        const [prods, cats, history] = await Promise.all([
          getProducts({ includeInactive: false }, true),
          getCategories(),
      ...
      <div className="pb-3 border-b border-[#EAE8E2] flex flex-col sm:flex-row sm:items-baseline justify-between gap-1">
        <div>
          ...
          <p className="text-xs text-[#75726B]">
            Katalog produk, harga modal, harga jual resmi, dan stok sistem
          </p>
        </div>
                ...
                <div>
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <span className="text-[10px] font-mono font-bold px-1.5 py-0.5 rounded bg-[#F5F4EE] text-[#121214] border border-[#E5E2DA]">
                        v{product.current_price_version}
                      </span>
                      {product.sku && (
                        <span className="text-[10px] font-mono font-medium px-1.5 py-0.5 rounded bg-[#FAF9F5] text-[#75726B] border border-[#E5E2DA]">
                          {product.sku}
                        </span>
                      )}
                    </div>
                ...
                <div className="mt-3 pt-3 border-t border-[#EAE8E2] flex items-end justify-between">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] text-[#85827B]">Modal:</span>
                      <span className="text-xs font-semibold text-[#605D57] font-mono">
                        {formatRupiah(product.purchase_price)}
                      </span>
                    </div>

                    <div>
                      <span className="text-[10px] text-[#85827B] mr-1.5">Jual:</span>
                      {isPriceChanged && visuals ? (
                        ...
                        <span className="text-sm font-extrabold font-mono text-[#121214]">
                          {formatRupiah(product.selling_price)}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="text-right">
                    <span className="text-[10px] text-[#85827B] block">Stok</span>
